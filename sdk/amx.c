...
#define DBGPARAM(v)     ( (v)=*(cell *)(code+(int)cip), cip+=sizeof(cell) )
    ...
    int AMXAPI npcamx_Cleanup(AMX *amx) {
            ...
            for (int i = 0; i < numlibraries; i++) {
                ...
                char funcname[sNAMEMAX + 12] = "amx_"; // +1 for '\0', +4 for 'amx_', +7 for 'Cleanup'
                strcat(funcname, GETENTRYNAME(hdr, lib));
    ...
    int AMXAPI npcamx_Clone(AMX *amxClone, AMX *amxSource, void *data) {
        if (!amxSource || !amxClone || !data)
            return amxSource ? (amxClone ? AMX_ERR_PARAMS : AMX_ERR_FORMAT) : AMX_ERR_FORMAT;

        if (!(amxSource->flags & AMX_FLAG_RELOC))
            return AMX_ERR_INIT;
        ...
        if (hdr->file_version > CUR_FILE_VERSION || hdr->amx_version < MIN_FILE_VERSION || (hdr->file_version == 9 && hdr->amx_version == 9))
            return AMX_ERR_VERSION;

        // Initialize clone
        amxClone->base = amxSource->base;
        amxClone->hlw = hdr->hea - hdr->dat;
        ...
        unsigned char _FAR *dataSource = amxSource->data ? 
            amxSource->data : amxSource->base + (int)hdr->dat;
        
        const size_t dataSize = (size_t)(hdr->hea - hdr->dat);
        memcpy(amxClone->data, dataSource, dataSize);
    ...
    int AMXAPI npcamx_NameLength(AMX *amx, int *length) {
        ...
        *length = USENAMETABLE(hdr) ? *(uint16_t*)(amx->base + (unsigned)hdr->nametable) : hdr->defsize - sizeof(ucell);
    ...
    int AMXAPI npcamx_NumNatives(AMX *amx, int *number) {
        ...
        if (!hdr || hdr->magic != AMX_MAGIC || hdr->natives > hdr->libraries)
            return AMX_ERR_FORMAT;
    ...
    int AMXAPI npcamx_GetNative(AMX *amx, int index, char *funcname) {
        ...
        AMX_FUNCPART *func = GETENTRY(hdr, natives, index);
    ...
    int AMXAPI npcamx_FindNative(AMX* amx, const char* name, int* index) {
        ...
        const int entries = NUMENTRIES(hdr, natives, libraries);
        if (!entries) {
            *index = INT_MAX;
        ...
        for (int idx = 0; idx < entries; idx++) {
            AMX_FUNCPART* func = GETENTRY(hdr, natives, idx);

            if (strcmp(name, GETENTRYNAME(hdr, func)) == 0) {
                *index = idx;
    ...
    int AMXAPI npcamx_NumPublics(AMX *amx, int *number) {
        ...
        if (!hdr || hdr->magic != AMX_MAGIC || hdr->publics > hdr->natives)
            return AMX_ERR_FORMAT;
    ...
    int AMXAPI npcamx_GetPublic(AMX *amx, int index, char *funcname) {
        ...
        AMX_FUNCPART *func = GETENTRY(hdr, publics, index);
        strcpy(funcname, GETENTRYNAME(hdr, func));
    ...
    int AMXAPI npcamx_NumPubVars(AMX *amx, int *number) {
        ...
        if (!hdr || hdr->magic != AMX_MAGIC || hdr->pubvars > hdr->tags)
            return AMX_ERR_FORMAT;
    ...
    int AMXAPI npcamx_GetPubVar(AMX *amx, int index, char *varname, cell *amx_addr) {
        ...
        AMX_FUNCPART *var = GETENTRY(hdr, pubvars, index);
    ...
    int AMXAPI npcamx_NumTags(AMX *amx, int *number) {
        ...
        *number = (hdr->file_version < 7) ? NUMENTRIES(hdr, tags, cod) : NUMENTRIES(hdr, tags, nametable);
    ...
    int AMXAPI npcamx_GetTag(AMX *amx, int index, char *tagname, cell *tag_id) {
        ...
        const int max_entries = (hdr->file_version < 7) ? NUMENTRIES(hdr, tags, cod) : NUMENTRIES(hdr, tags, nametable);

        if (index >= max_entries)
            return AMX_ERR_INDEX;

        AMX_FUNCPART *tag = GETENTRY(hdr, tags, index);
    ...
    int AMXAPI npcamx_GetUserData(AMX *amx, long tag, void **ptr) {
        ...
        for (int index = 0; index < AMX_USERNUM; index++) {
            if (amx->usertags[index] == tag) {
    ...
    int AMXAPI npcamx_SetUserData(AMX *amx, long tag, void *ptr) {
        ...
        int index;
        for (index = 0; index < AMX_USERNUM; index++) {
            if (amx->usertags[index] == tag)
                break;
        }
    ...
    int AMXAPI npcamx_Register(AMX *amx, const AMX_NATIVE_INFO *list, int number) {
        ...
        for (int i = 0; i < numnatives; i++) {
            if (func->address == 0) {
                AMX_NATIVE funcptr = list ? findfunction(GETENTRYNAME(hdr, func), list, number) : NULL;
...
#define STKMARGIN ((cell)(16*sizeof(cell)))
    ...
    int AMXAPI npcamx_Push(AMX *amx, cell value) {
        if (!amx || amx->hea + STKMARGIN > amx->stk)
    ...
    int AMXAPI npcamx_PushArray(AMX *amx, cell *amx_addr, cell **phys_addr, const cell array[], int numcells) {
        if (!amx || !array || numcells <= 0)
            return AMX_ERR_PARAMS;

        cell xaddr;
        cell *paddr;
        int err = npcamx_Allot(amx, numcells, &xaddr, &paddr);
    ...
    int AMXAPI npcamx_PushStringLen(AMX* amx, cell* amx_addr, cell** phys_addr, const char* string, int length, int pack, int use_wchar) {
        if (!amx || !string || length < 0)
            return AMX_ERR_PARAMS;

        const int numcells = pack ? (length + 1 + sizeof(cell) - 1) / sizeof(cell) : length + 1;
    ...
    int AMXAPI npcamx_PushString(AMX *amx, cell *amx_addr, cell **phys_addr, const char *string, int pack, int use_wchar) {
        ...
        const size_t length = 
            ...
            strlen(string);
        #else
            use_wchar ? wcslen((const wchar_t*)string) : strlen(string);
        #endif
    ...
    */
    ...
    {
        if (!amx || !amx_addr || !phys_addr || cells <= 0)
            return AMX_ERR_PARAMS;
        ...
        const size_t required_space = cells * sizeof(cell);

        if (amx->stk - amx->hea - required_space < STKMARGIN)
            return AMX_ERR_MEMORY;

        unsigned char *data = amx->data ? amx->data : amx->base + (int)hdr->dat;
        *amx_addr = amx->hea;
        *phys_addr = (cell *)(data + (int)amx->hea);
        amx->hea += required_space;
    ...
    int AMXAPI npcamx_StrLen(const cell *cstr, int *length)
    {
        ...
        if ((ucell)*cstr > UNPACKEDMAX) {
            // Packed string handling
            len = strlen((char *)cstr);
            
            #if BYTE_ORDER == LITTLE_ENDIAN
                cell c = cstr[len/sizeof(cell)];
                len -= len % sizeof(cell);
    ...
    static inline void swap_cell(ucell *c)
    {
        unsigned char *ptr = (unsigned char *)c;
        unsigned char temp;
        
        temp = ptr[0];
        ptr[0] = ptr[sizeof(cell)-1];
        ptr[sizeof(cell)-1] = temp;
        
        temp = ptr[1];
        ptr[1] = ptr[sizeof(cell)-2];
        ptr[sizeof(cell)-2] = temp;
    }
    ...
    int AMXAPI npcamx_SetStringLen(cell* dest, const char* source, int length, int pack, int use_wchar, size_t size)
    {
        ...
        if (pack) {
            ...
            #if BYTE_ORDER == LITTLE_ENDIAN
                for (size_t i = 0; i <= len / sizeof(cell); i++)
                    swap_cell((ucell*)&dest[i]);
            #endif
        }
        else {
            // Handle unpacked string
    ...
    int AMXAPI npcamx_UTF8Get(const char *string, const char **endptr, cell *value)
    {
        ...
        if (c < 0x80)
            ...
            int followup = utf8_count[(c >> 2) & 0x0f];
            result = c & (0x1f >> followup);
            ...
            if (result < utf8_lowmark[followup] || (result >= 0xd800 && result <= 0xdfff) || result == 0xfffe || result == 0xffff)
                return AMX_ERR_PARAMS;
        }
    ...
    int AMXAPI npcamx_UTF8Check(const char *string, int *length)
    {
        ...
        while (err == AMX_ERR_NONE && *ptr != '\0') {
            err = npcamx_UTF8Get(ptr, &ptr, NULL);
            len++;
        }
    ...
    int AMXAPI npcamx_UTF8Len(const cell *cstr, int *length)
    {
        ...
        int err = npcamx_StrLen(cstr, length);
        ...
        if ((ucell)*cstr <= UNPACKEDMAX) {
            ...
            while (len-- > 0) {
                err = npcamx_UTF8Put(buffer, &endptr, sizeof(buffer), *cstr++);
