...
Макрос `Plugin_Module` позволяет вашему плагину выступать в качестве "загрузчика" для других плагинов, создавая модульную и расширяемую архитектуру.

#### Синтаксис и назначение

- `Plugin_Module(const char* имя_базового_файла, const char* каталог_модуля, const char* необязательное_сообщение_об_успехе)`
- `имя_базового_файла`: **Базовое** имя файла модуля, **без расширения** (например, для `my_module.dll` или `my_module.so` используйте `"my_module"`). SDK автоматически добавит соответствующее расширение `.dll` или `.so`.
- `каталог_модуля`: Путь к каталогу, где расположен файл модуля (например, `"plugins/my_custom_modules"`). **Не включайте имя файла здесь.** SDK выполнит конкатенацию полного пути (`каталог_модуля/имя_базового_файла.расширение`).
- `необязательное_сообщение_об_успехе`: Необязательное сообщение, которое будет записано в консоль сервера, если модуль успешно загружен.
...
if (!Plugin_Module("core_logic", "modules/custom", "Модуль основной логики успешно загружен!"))
...
if (!Plugin_Module("admin_system", "plugins", "Модуль администрирования активирован."))
    Samp_SDK::Log("ПРЕДУПРЕЖДЕНИЕ: Не удалось загрузить модуль администрирования.");
```

#### Жизненный цикл модуля

- **Загрузка:** Когда вызывается `Plugin_Module`, SDK:
   1. Составляет полный путь к файлу (например, `plugins/custom/core_logic.dll`).
   2. Использует `Dynamic_Library` (`LoadLibrary`/`dlopen`) для загрузки бинарника.
   3. Получает указатели на функции жизненного цикла модуля: `Load`, `Unload` и `Supports`.
...
- **Выгрузка:** Во время `OnUnload` вашего основного плагина SDK выгружает все модули, которые были загружены через `Plugin_Module`. Это делается в **обратном порядке** загрузки (последний загруженный выгружается первым), что крайне важно для управления зависимостями и обеспечения правильного освобождения ресурсов.

#### Преимущества модульности

- **Организация кода:** Разделите большие плагины на более мелкие, управляемые компоненты, каждый в своем собственном файле модуля.
...
- **Динамические обновления:** В контролируемых сценариях позволяет обновлять части вашей системы (заменяя `.dll` или `.so` модуля) без необходимости перекомпиляции и перезапуска основного плагина или всего сервера (хотя это требует строгого управления версиями и совместимостью).
