...
- [SA-MP SDK](#sa-mp-sdk)
  ...
  - [4. Внутреннее строение и архитектура SDK](#4-внутреннее-строение-и-архитектура-sdk)
    ...
    - [4.8. `native_hook_manager.hpp`: Движок хуков Natives](#48-native_hook_managerhpp-движок-хуков-natives)
...
- **Макросы оптимизации и предсказания ветвлений:**
   - `SAMP_SDK_FORCE_INLINE`:
      - **Механизм:** `__forceinline` (MSVC) или `__attribute__((always_inline)) inline` (GCC/Clang). Настоятельно предлагает компилятору встроить тело функции непосредственно в место вызова, устраняя накладные расходы на реальный вызов функции.
      - **Использование:** Применяется к небольшим и критически важным для производительности функциям внутри SDK.
   - `SAMP_SDK_LIKELY(x)` / `SAMP_SDK_UNLIKELY(x)`:
      - **Механизм:** `[[likely]]` / `[[unlikely]]` (C++20) или `__builtin_expect` (GCC/Clang). Подсказки компилятору о том, какой путь `if/else` наиболее вероятно будет выбран.
      - **Воздействие:** Помогает компилятору генерировать более эффективный код для предсказания ветвлений (branch prediction), уменьшая задержку ЦП.
      - **Пример (`platform.hpp`):**
         ```cpp
...
### 4.8. `native_hook_manager.hpp`: Движок хуков Natives

Это надежная система хукинга нативных функций, разработанная для управления цепочкой хуков из нескольких плагинов для одной и той же нативной функции.

- **`Native_Hook`**:
   - **Описание:** Класс, представляющий один хук нативной функции. Хранит хеш имени нативной функции, функцию-обработчик C++, предоставленную пользователем (`user_handler_`), и `std::atomic<AMX_NATIVE> next_in_chain_`.
   - **`user_handler_`**: Ваша функция C++ `Plugin_Native_Hook`.
   - **`next_in_chain_`**: Указатель на исходную native или на хук плагина с более низким приоритетом. Это `std::atomic` для обеспечения потокобезопасности при чтении/записи.
   - **`Dispatch(AMX* amx, cell* params)`**: Вызывается трамплином для выполнения вашего `user_handler_`.
   - **`Call_Original(AMX* amx, cell* params)`**: Критически важный метод, который вызывает `next_in_chain_`, позволяя вашему хуку вызывать исходную функциональность или следующий хук в цепочке.
- **`Trampoline_Allocator`**:
   - **Описание:** Класс, отвечающий за выделение блоков исполняемой памяти и генерацию кода ассемблерного "трамплина" в этих блоках.
   - **`Generate_Trampoline_Code(unsigned char* memory, int hook_id)`**: Записывает 10 байт ассемблерного кода:
      1. `B8 XX XX XX XX`: `MOV EAX, hook_id` (помещает уникальный идентификатор хука в регистр EAX).
   ...
   - **Разрешения памяти:** Использует `VirtualAlloc` (Windows) или `mmap` (Linux) с разрешениями `EXECUTE_READWRITE`, чтобы гарантировать возможность выполнения сгенерированного кода.
- **`Dispatch_Wrapper_Asm()`**:
   - **Описание:** Небольшая функция на ассемблере (определенная с `__declspec(naked)` или `asm volatile`), которая служит целью для всех трамплинов.
   - **Действие:** Сохраняет регистры, перемещает `EAX` (который содержит `hook_id`) в стек и вызывает функцию `Dispatch_Hook` на C++. После возврата из `Dispatch_Hook` восстанавливает регистры и возвращается.
...
- **`Native_Hook_Manager`**:
   ...
   - **`std::unordered_map<uint32_t, Trampoline_Func> hash_to_trampoline_`**: Сопоставляет хеш имени native с указателем на сгенерированный трамплин.
   - **`std::vector<uint32_t> hook_id_to_hash_`**: Сопоставляет целочисленный ID хука (используемый в трамплине) обратно с хешем имени native.
   - **`Get_Trampoline(uint32_t hash)`**: Возвращает (или создает и выделяет) указатель на трамплин для данного хеша native.
- **`PLUGIN_NATIVE_HOOK_REGISTRATION`**:
   - **Механизм:** Макрос, который создает статический глобальный класс (`Native_Hook_Register_##name`) для каждого `Plugin_Native_Hook`. В статическом конструкторе этого класса он регистрирует обработчик пользователя в `Native_Hook_Manager`.
